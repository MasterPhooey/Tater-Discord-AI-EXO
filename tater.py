...
load_dotenv()
exo_model = os.getenv("EXO_MODEL", "llama-3.1-8b").strip()
...
class tater(commands.Bot):
    def __init__(self, *args, **kwargs):
        ...
        self.model = exo_model  # Using EXO model

    async def exo_chat(self, messages: list, stream: bool = False):
        """Call the EXO API and return the JSON response."""
        payload = {
    ...
    async def generate_error_message(self, prompt: str, fallback: str, message: discord.Message):
        """
        Uses EXO to generate a friendly error message based on a prompt.
        Returns the generated message or the fallback text if generation fails.
        """
        try:
            error_response = await self.exo_chat([{"role": "system", "content": prompt}], stream=False)
            error_text = error_response["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()
            if error_text:
                return error_text
        except Exception as e:
    ...
    async def on_message(self, message: discord.Message):
        ...
        # DM logic: only admin in DMs gets a response.
        ...
        # Generate embedding and store message if long enough.
        embedding = None
        ...
        system_prompt = (
            ...
            "The DNServ Crew is an elite, tight-knit Retro Gaming group. You help the DNServ Crew with various tools. If you need real-time access to the internet or lack information about what the user is asking, use the 'web_search' tool. Don't ask to use a tool—just use it.\n\n"
            ...
            "6. 'watch_feed' for adding a new RSS feed to monitor. When a new article appears, the bot will summarize it and announce the news.\n\n"
            "7. 'unwatch_feed' for stopping the monitoring of an RSS feed.\n\n"
            "8. 'list_feeds' for listing all currently watched RSS feeds.\n\n"
            "9. 'web_search' for searching the web when additional or up-to-date information is needed to answer a user's question. If you are unsure of the answer or your knowledge is outdated, use this tool to retrieve current information from the web.\n\n"
        ...
        async with message.channel.typing():
            try:
                logger.debug(f"Sending request to EXO with messages: {messages_list}")
                response_data = await self.exo_chat(messages_list, stream=False)
                logger.debug(f"Raw response from EXO: {response_data}")

                response_text = response_data["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()

                if not response_text:
                    logger.error("EXO returned an empty response.")
                    ...
                    if response_json["function"] == "youtube_summary":
                        ...
                        if video_url:
                            ...
                            if video_id:
                                ...
                                waiting_response = await self.exo_chat(
                                ...
                                if article:
                                    ...
                                    prompt = f"Generate an error message to {message.author.mention} explaining that I was unable to retrieve the summary from the YouTube video. Only generate the message. Do not respond to this message."
                                    error_msg = await self.generate_error_message(prompt, "Failed to retrieve the summary from YouTube.", message)
                                    await message.channel.send(error_msg)
                            else:
                                prompt = f"Generate an error message to {message.author.mention} explaining that the provided YouTube URL is invalid. Only generate the message. Do not respond to this message."
                                error_msg = await self.generate_error_message(prompt, "The provided YouTube URL is invalid.", message)
                                await message.channel.send(error_msg)
                        else:
                            prompt = f"Generate an error message to {message.author.mention} explaining that no YouTube URL was provided in the function call. Only generate the message. Do not respond to this message."
                    ...
                    # --- Web Summary ---
                        ...
                        if webpage_url:
                            ...
                            if summary:
                                ...
                                prompt = f"Generate an error message to {message.author.mention} explaining that I was unable to retrieve the summary from the webpage. Only generate the message. Do not respond to this message."
                                error_msg = await self.generate_error_message(prompt, "Failed to retrieve the summary from the webpage.", message)
                                await message.channel.send(error_msg)
                        else:
                            prompt = f"Generate an error message to {message.author.mention} explaining that no webpage URL was provided in the function call. Only generate the message. Do not respond to this message."
                        ...
                        if prompt_text:
                            ...
                            async with message.channel.typing():
                                ...
                                try:
                                    ...
                                    prompt = f"Generate an error message to {message.author.mention} explaining that I was unable to create the image. Only generate the message. Do not respond to this message."
                                    error_msg = await self.generate_error_message(prompt, f"Failed to generate image: {e}", message)
                                    await message.channel.send(error_msg)
                        else:
                            prompt = f"Generate an error message to {message.author.mention} explaining that no prompt was provided for drawing a picture. Only generate the message. Do not respond to this message."
                        ...
                        if message.attachments:
                            ...
                            async with message.channel.typing():
                                try:
                                    ...
                                    prompt = f"Generate an error message to {message.author.mention} explaining that I was unable to retrieve the Premiumize download links for the torrent. Only generate the message. Do not respond to this message."
                        ...
                        if feed_url:
                            if hasattr(self, "rss_manager") and self.rss_manager is not None:
                                ...
                                if success:
                                    ...
                                    generated = await self.exo_chat(
                                        [{"role": "system", "content": prompt}],
                                        stream=False
                                    )
                                    confirmation_text = generated["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()
                    ...
                    # --- List Feeds ---
                        ...
                        if hasattr(self, "rss_manager") and self.rss_manager is not None:
                            ...
                            if feeds:
                                ...
                                response_text = generated["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()
                                if response_text:
                    ...
                    # --- Web Search ---
                        ...
                        if query:
                            ...
                            if results:
                                ...
                                choice_response = await self.exo_chat(
                                    [{"role": "system", "content": choice_prompt}],
                                    stream=False
                                )
                                choice_text = choice_response["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()
                                ...
                                if choice_json.get("function") == "web_fetch":
                                    ...
                                    if link:
                                        ...
                                        if summary:
                                            ...
                                            final_response = await self.exo_chat(
                                                [{"role": "system", "content": info_prompt}],
                                                stream=False
                                            )
                                            final_answer = final_response["choices"][0]["message"]["content"].replace("<|im_end|>", "").strip()
                                            if final_answer:
